...
//Carrega o terreno onde a altura do terreno varia de -height / 2 atÃ© height / 2
...
void handleCollisions(vector<Animal*> &animals,
	...
	//Calcula as potenciais colisÃµes de acordo com o quadrante
			...
			if (a1->type() == a2->type()) {
				...
				if (a1->type() == ZEBRA && a2->type() == LION) {
					if (a1->scale()-0.88 > 1.5 * a2->scale()) {
						...
						//NecessÃ¡rio para evitar o segmentation fault 11
